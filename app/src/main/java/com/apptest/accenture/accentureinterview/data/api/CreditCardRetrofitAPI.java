...
public interface CreditCardRetrofitAPI {
    ...
    Call<ModelCreditCard> checkCreditCard(@Path("creditcard") String creditcard);

    @GET("getallcreditcards")
    Call<ArrayList<ModelCreditCard>> getAllCreditCards();

    @POST("savecreditcard")
    Call<ModelCreditCard> saveCreditCard(@Body ModelCreditCard modelCreditCard);
}
